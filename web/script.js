...
var Atrament = /** @class */ (function () {
    function Atrament(selector, width, height, color) {
        ...
        this.canvas.width = width || 28;
        this.canvas.height = height || 28;
    ...
    Atrament.colorPixel = function (data, fillR, fillG, fillB, startColor, alpha) {
        var matchColor = Atrament.matchColor.apply(Atrament, __spreadArrays([data], startColor));
        return function (pixelPos) {
    ...
    Atrament.prototype.fill = function () {
        ...
        if (!this._filling) {
            ...
            this._filling = true;
            setTimeout(function () { _this._floodFill(mouse.x, mouse.y, startColor); }, 28);
        }
        else {
    ...
    Atrament.prototype._floodFill = function (startX, startY, startColor) {
        ...
        var colorPixel = Atrament.colorPixel.apply(Atrament, __spreadArrays([colorLayer.data], fillColor, [startColor, alpha]));
        var matchColor = Atrament.matchColor.apply(Atrament, __spreadArrays([colorLayer.data], startColor));
        var matchFillColor = Atrament.matchColor.apply(Atrament, __spreadArrays([colorLayer.data], __spreadArrays(fillColor, [255])));
        // check if we're trying to fill with the same colour, if so, stop
        if (matchFillColor((startY * context.canvas.width + startX) * 4)) {
            this._filling = false;
            setTimeout(function () { _this.canvas.style.cursor = 'crosshair'; }, 28);
            return;
        }
    ...
    Atrament.prototype.scaleDown = function () {
        ...
        context.fillText("1", (canvas.width / 2) - 5, (canvas.height / 2) + 8);
        ...
        // originalCanvas.width = boundedCanvas.width = centeredCanvas.width = scaledCanvas.width = 28;
        // originalCanvas.height = boundedCanvas.height = centeredCanvas.height = scaledCanvas.height = 28;
        ...
        for (var i_1 = 0; i_1 < originalCanvas.width * originalCanvas.height * 4; i_1 += 4) {
            ...
            pixels[row].push(Math.round(a * 28) / 28);
            column++;
        }
        ...
        // //     var img = centeredCtx.createImageData(28, 28);
        // //     for (var i = img.data.length; --i >= 0; )
        // //       img.data[i] = 0;
        // //     centeredCtx.putImageData(img, 28, 28);
        ...
        // Reset the drawing.
        var img = mnistCtx.createImageData(28, 28);
        for (var i = img.data.length; --i >= 0;)
            img.data[i] = 0;
